...
export default {
  ...
  methods: {
    deleteBatch() {
      ...
      this.$axios.post("/slide/del/batch", ids)
        .then(res => {
          this.loading = false
          if (res.code == 200) {
    ...
    getData() {
      this.loading = true
      this.$axios.get(`/slide/page?pageNum=${this.currentPage}&pageSize=${this.pageSize}&name=${this.searchForm.name}`).then(res => {
        this.loading = false
        if (res.code != 200) {
          this.$message({
            type: 'info',
    ...
    updateData() {
      this.$refs.editForm.validate((valid) => {
        ...
        this.$axios.post("/slide", this.editForm)
          .then(res => {
            this.loading = false
            if (res.code == 200) {
              this.editFormVisible = false;
